...
/// TDD Green Phase: ItemDetailFeatureのテスト
///
/// ItemDetailFeature Reducerが実装され、テストが成功するはずです。
...
struct ItemDetailFeatureTests {
    ...
    func startEditing() async throws {
        let store = await TestStore(initialState: ItemDetailFeature.State(item: sampleItem)) {
            ItemDetailFeature()
        }
